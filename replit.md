...
- `carbongpt/core/evidence_engine.py` — Evidence linking (parameters/sections to source documents), evidence completeness scoring, citation generation. Phase 2c Evidence Pipeline MVP: manual `extract_parameter_evidence()` for 9 critical params (fNRB, num_devices, num_households, household_size, baseline/project_fuel_consumption, usage_rate, SFC_baseline, SFC_project), `decide_evidence()` with accept/reference/reject decisions, confirmation guard for overwriting confirmed values, dedup on param_key+value+doc+page, `get_pending_evidence()`, `get_evidence_counts_by_param()`, `decide_evidence_force()` for confirmed overwrite. Extraction quality: chunked processing (12k chars/chunk, 1.5k overlap) for long documents, anti-hallucination prompt (no current values leaked, verbatim quote requirement), quote validation against source text, noise pattern detection for survey samples/job counts, unit mismatch detection, range validation with percent normalization
