...
-   **Pre-parsed Methodology Cache (`methodology_parsed` table):** Methodologies are parsed once and stored in the database. When a user opens the Calculations tab, the pre-parsed data loads instantly from DB — no AI call needed. Admin can trigger batch parsing via `POST /admin/methodology-parsed/batch` or parse individual methodologies via `POST /admin/methodology-parsed/parse?methodology_code=X`. If a methodology hasn't been pre-parsed yet, the user can trigger on-demand parsing via the UI (fallback). Key functions: `get_or_parse_methodology()` (load from cache or parse), `parse_methodology_and_save()` (parse and store), `batch_parse_methodologies()` (batch all docs).
...
-   **API Endpoints:** `GET /projects/{id}/methodology-data` (instant load from cache), `POST /projects/{id}/parse-methodology`, `POST /projects/{id}/calculate`, `POST /projects/{id}/export-calculation`, `POST /projects/{id}/generate-template`, `GET /admin/methodology-parsed` (list all parsed), `POST /admin/methodology-parsed/parse` (parse one), `POST /admin/methodology-parsed/batch` (parse all)
-   **UI Tabs:** "Calculations" tab (auto-loads pre-parsed methodology, enter inputs, run calcs, view results) and "Export" tab (download Word/Excel) in the project workspace.