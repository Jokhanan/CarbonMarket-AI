...
- `carbongpt/core/evidence_engine.py` — Methodology-agnostic evidence extraction engine. All param aliases, noise terms, extraction hints, and baseline/project categories come from `PARAMETER_DEFINITIONS` in `parameter_engine.py` via `get_extraction_metadata(methodology)`. No hard-coded methodology-specific constants. Features: `extract_parameter_evidence()` (dynamic param selection per methodology), `decide_evidence()` with accept/reference/reject decisions, confirmation guard for overwriting confirmed values, dedup on param_key+value+doc+page, `get_pending_evidence()`, `get_evidence_counts_by_param()`, `decide_evidence_force()` for confirmed overwrite. Extraction quality: chunked processing (12k chars/chunk, 1.5k overlap) for long documents, anti-hallucination prompt (no current values leaked, verbatim quote requirement), dynamic baseline/project guidance blocks, per-param extraction hints, quote validation against source text, noise pattern detection from metadata, unit mismatch detection, range validation with percent normalization
...
- `evidence_links` — Links claims/parameters to source documents/chunks. Phase 2c adds: `extracted_value`, `extracted_unit`, `param_key`, `evidence_type` (default 'parameter_value'), `evidence_decision` (pending/accepted/accepted_as_reference/rejected/superseded)
