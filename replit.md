...
-   **AI Writing - Template Compliance:** All 7 guide files include `content_format` and `format_instructions` fields per section, driving the AI to produce template-compliant output (tables, parameter blocks, checklists, equations) instead of defaulting to prose. Format types: prose, table, parameter_blocks, checklist, equations_and_prose, summary_table.
...
-   **Document Exporter:** Exports calculation results to Excel and generates filled Word templates.
